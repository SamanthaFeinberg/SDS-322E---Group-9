...
Traffic_Counts <- Traffic_Counts %>%
     mutate(modified_date = as.Date(modified_date, format = "%Y-%m-%d"),
            current_date = Sys.Date(),
            detector_age = as.numeric(difftime(current_date, modified_date, units = "days")))
   Traffic_Counts$operational_status_num <- ifelse(Traffic_Counts$detector_status == "OK", 1, 0)
   
   model <- lm(operational_status_num ~ detector_age, data = Traffic_Counts)
   summary(model)
